...
const CreateInvoice = ({ existingInvoice, onSave }) => {
  ...
  const [isGeneratingNumber, setIsGeneratingNumber] = useState(!existingInvoice); // Fix: correct spelling
      ...
      const generateNewInvoiceNumber = async () => {
        ...
        try {
          ...
          invoices.forEach((inv) => {
            const num = parseInt(inv.invoiceNumber.split("_")[1]);
  ...
  return <form onSubmit={handleSubmit} className="space-y-8 pb-[100vh]">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
      <div className="bg-white p-6 rounded-lg shadow-sm shadow-gray-100 border border-slate-200 space-y-4">
        ...
        <InputField label="Address" name="address" value={formData.billFrom.address} onChange={(e) => handleInputChange(e, "billFrom")} />
        ...
        <table className="w-full divide-y divide-slate-200">
          <thead className="bg-slate-50">
            <tr>
              ...
              <th className="px-2 sm:px-6 py-3">Item</th>
            </tr>
          </thead>
          <tbody className="bg-white divide-y divide-slate-200">
            {formData.items.map((item, index) => {
              <tr key={index} className="hover:bg-slate-50">
                ...
                <td className="px-2 sm:px-6 py-4">
                  <input type="number" name="texPercent" value={item.taxPercent} onChange={(e) => handleInputChange(e, null, index)} className="w-full h-10 px-3 py-2 border border-slate-200 rounded-lg bg-white text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="0" />
                </td>
    ...
    <div className="">
      ...
      <div className="">
        <div className="">
          <div className=""><p>SubTotal:</p><p>${subtotal.toFixed(2)}</p></div>
        </div>
      </div>
    </div>
  </form>
